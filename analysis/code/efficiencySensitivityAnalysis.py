...
print("--- Loading Efficiency Sensitivity Analysis Data ---")
...
if not all_data:
    ...
    formula = 'round ~ C(Group) * C(Decay_Rate)'
    lm = ols(formula, data=df_combined).fit()

    print("\n--- Two-Way ANOVA Results for Negotiation Efficiency (Round Number) ---")
    anova_table = sm.stats.anova_lm(lm, typ=2)
    print(anova_table)

    anova_table.to_csv("analysis/ANOVAsensitivityEfficiency2Way.csv")
    ...
    ax = sns.pointplot(data=df_combined, x='Decay_Rate', y='round', hue='Group',
                       errorbar=('ci', 95), dodge=0.1, capsize=0.05, palette="viridis",
                       markers=['o', 's'], linestyles=['-', '--'])

    plt.title('Impact of Information Group and Decay Rate on Negotiation Efficiency', fontsize=16, fontweight='bold')
    plt.xlabel('Urgency Condition (Decay Rate)', fontsize=14)
    plt.ylabel('Mean Round Number', fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig("analysis/EfficiencySensitivityAnalysis.pdf", dpi=300, bbox_inches='tight')
    plt.show()

    print(f"Results saved to analysis/ANOVAsensitivityEfficiency2Way.csv and analysis/EfficiencySensitivityAnalysis.pdf")
